...
allocate(i_ph_int(4,-1:1))
 ...
 write(*,*)'Dimension of reduced space =',dim_baser

 write(*,*)' CM SVD orthogonalisation?  y/n'
 read(*,*)yesno
 if (yesno == 'n') phonbs%spur=0
...
do i=1,dim_baser
 do j=i,dim_baser
   if (dabs(ham(i,j)-ham(j,i)).gt.0.00001d0) write(*,'(2i5,3f10.5)'),i,j,dabs(ham(i,j)-ham(j,i))
 enddo
enddo
...
do i=1,dim_ind
 do j=i,dim_ind
   if (dabs(ham(i,j)-ham(j,i)).gt.0.0001d0) write(*,'(2i5,3f10.5)'),i,j,dabs(ham(i,j)-ham(j,i))
 enddo
enddo
