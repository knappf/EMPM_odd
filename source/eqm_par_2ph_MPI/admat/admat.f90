...
!     i=ind_red(ii)
  ...
  i_lam=phonbs(i)%ila 
     ...
     if (i_lam.ne.i_lam_old) then 
          call readro11('2f_rp.dat',i_lam,ronh1,dim_phon,1,nlev,1,nlev,isi_max)
          i_lam_old=i_lam
     endif 
...
!     ifaz=(lev(j_sp)%j-j_tot)/2+phon(j_lam)%j
     ...
     do isi=0,isi_max
       d_mat(j)=d_mat(j)-dfloat(ifaz)*ronh1(j_lam,isi,i_sp,j_sp)*(2*isi+1)**0.5d0*racah(lev(i_sp)%j,lev(j_sp)%j,2*phon(i_lam)%j,2*phon(j_lam)%j,2*isi,j_tot)
     enddo   
...
type(phon_typ), dimension (:), allocatable :: phon
type(phonbase_typ), dimension (:), allocatable :: phonbs
type(ro2_typ), dimension(:), allocatable :: ronp,ronh
      ...
      if (i_sp.eq.j_sp.and.i_lam.eq.j_lam) then 
       ...
       do iii=1,ndbr(1,j_lam)  !nronp          ! neutron particle
         ii=ipozbr(1,j_lam,iii)
          ...
          a_mat(j)=a_mat(j)+rac*ifaz*(2*isi+1)**0.5d0*0.5d0*ronp(ii)%ro*fp(isi,i_sp,j_sp,i2,i1)
        enddo
       ...
       do iii=1,ndbr(2,j_lam) ! nronh          ! neutron hole
         ii=ipozbr(2,j_lam,iii)
          ...
          ifaz=(-1)**ifaz
          rac=racah(2*phon(i_lam)%j,2*isi,j_tot,lev(j_sp)%j,2*phon(j_lam)%j,lev(i_sp)%j)
          a_mat(j)=a_mat(j)+rac*ifaz*(2*isi+1)**0.5d0*ronh(ii)%ro*fp(isi,i_sp,j_sp,i2,i1)
        enddo
