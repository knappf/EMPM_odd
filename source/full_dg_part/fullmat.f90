...
! if (tzcal.eq.-1) then    
   do i=1,ip_lev
     iparl=(-1)**lev(par_lev(i))%l
!     if (lev(hol_lev(i)%j.eq.jcal.and.iparl.eq.iparc) then  
     phonbs(0,i)%par=iparl
     phonbs(0,i)%jj=lev(par_lev(i))%j
     phonbs(0,i)%tz=1  ! neutron particle 
     if (mod(par_lev(i),2).ne.0) phonbs(0,i)%tz=-1   ! proton particle 
     phonbs(0,i)%enf=lev(par_lev(i))%en
!     endif
   enddo
 ...
 if (nfon.gt.0) then
        ...
        if (nfon.eq.1) namef='1phon_part/1f_states.dat'
...
subroutine inter_par_phon1(ndmpho,nbs,xamp,ndx,vlam,jcal,par_lev,hol_lev)
implicit none 
...
integer :: i,ih,ip,ila,jsp,i_jsp,ic,ipp,ihh,jc,inu,iu,jcal,ifaz,jp,nlev,jmax
...
!    vlam=0.d0
    do ic=1,ndc(ila)
      ...
      jc=jphon(ila)
      ifaz=(-1)**((jcal-jp)/2-jc)
      vlam(iu,jsp)=vlam(iu,jsp)+ifaz*0.5d0*camp(ila,ic)%am*fp(jc,ipp,ihh,ip,i_jsp)*xamp(inu,i)%am*dfloat(2*jc+1)**0.5d0/(dfloat(jcal+1))
    enddo
...
return 
end subroutine inter_par_phon1
!--------------------------------------------------------------------------------------
